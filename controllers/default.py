            ...
            if a_convert.to_int(request.args[0]):
                ...
                if not post:
                    ...
                    if pg and len(pg)>0: pg = pg[0]
                        ...
                        redirect(URL(r=request, f='add', args=['page', request.args[0]]))
                        return
    ...
    try:
        ...
        if area == "post":
            ...
            if len(request.args)>1:
                db.posts.post_title.default = request.args[1]
        ...
        if edit_form.accepts(request.vars, session):
            if request.vars.delete_this_record=='on':
                ...
                if request.vars.post_attributes_json and request.vars.post_attributes_json!='':
                    ...
                    session.flash = T("Post updated. (%(view_link)s)" % 
                                    dict(view_link=A(T('view'), _href='javascript:window.open("%s", "%s");' % 
                                        (XML(URL(r = request, f='post', args=[id])),
                                        '%s_view_%s' % (host.replace(':', '-'), request.vars.post_title)))))
            ...
            if request.vars.delete_this_record=='on':
                ...
                try:
                    ...
                    if request.vars.post_attributes_json and request.vars.post_attributes_json!='':
                        ...
                        _json['content_is']['original']=request.vars.post_attributes_content_is_original=='on'
                        db(db.posts.id==id).update(post_attributes_json=simplejson.dumps(_json))
                    _target = '%s_view_%s' % (host.replace(':', '-'), request.vars.post_title)
                    session.flash = T("Page updated. (%(view_link)s)" % 
                                    dict(view_link=A(T('view'), _href='javascript:window.open("%s", "%s");' % 
                                        (XML(URL(r = request, f='page', args=[id])), _target), _target=_target)))
                    """             dict(view_link=A(T('view'), _href=URL(r = request, f='page', args=[id]),
                                     _target='%s_view_%s' % (host.replace(':', '-'), request.vars.post_title))))"""
                except Exception, ex:
                    session.flash=T("(Minor) Error occured: %(err)s ", dict(err=str(ex)))
                finally:
